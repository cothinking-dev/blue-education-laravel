...
@php
    $waNumber = config('seo.organization.whatsapp');
    $waDisplay = config('seo.organization.phone_mobile');
    ...
    <div class="relative max-w-7xl mx-auto px-8 lg:px-16 py-16 lg:py-20">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8 lg:gap-16">
            ...
            <div class="flex flex-col items-center lg:items-start gap-3 shrink-0">
                ...
                <a href="{{ $primaryHref }}" class="inline-flex text-white font-medium hover:text-primary-200 transition-colors text-sm">Or send an enquiry &rarr;</a>
                @if($secondaryText)
                    <a href="{{ $secondaryHref }}" class="inline-flex text-primary-200 hover:text-white transition-colors text-sm">{{ $secondaryText }}</a>
                @endif
                @if($phone)
                    <div class="text-sm space-y-1 mt-1">
                        <p><a href="{{ $waUrl }}" target="_blank" rel="noopener noreferrer" class="text-white hover:text-primary-200 font-medium">WhatsApp: {{ $waDisplay }}</a></p>
                        <p class="text-primary-300">Or call: <a href="tel:{{ preg_replace('/[^+0-9]/', '', $phone) }}" class="text-primary-200 hover:text-white">{{ $phone }}</a></p>
                    </div>
                @endif
            </div>
        </div>
